<?php 
/**
 * ApGridView class file.
 * 
 * Extension of CGridView to make it useable with
 * AlphaPagination.
 * 
 * @author Jascha Koch
...
Yii::import($alias.'.ApLinkPager');
Yii::import($alias.'.ApListPager');
//if you want, you can destroy an alias created
Yii::setPathOfAlias($alias,'');

class ApGridView extends TbExtendedGridView
{
	/**
	...
	public function renderAlphapager()
	{
		...
		if($this->ajaxUpdate!==false)
		{
			...
			{
				$cs->registerScript(__CLASS__.'#'.$id,"jQuery('#".$id." .".$this->alphaPagerCssClass." a').live('click',function(){ $.fn.yiiGridView.update('".$id."',{url: $(this).attr('href')});return false; });");
			}
			else if($class == 'ApListPager')
			{
				$cs->registerScript(__CLASS__.'#'.$id,"jQuery('#".$id." .".$this->alphaPagerCssClass." select').live('change',function(){ $.fn.yiiGridView.update('".$id."',{url: $(this).val()});return false; });");
